...
class UCB():
    ...
    def update(self, action, feedback, outcome, t, x_array):
        #outcome, t, x_array pas nécessaire
        self.u[action] = (self.u[action]*(self.N_k[action]-1) + feedback)/self.N_k[action]
        self.u[[1,0][action]] = (self.u[[1,0][action]]*(self.N_k[[1,0][action]]-1) + [1,0][feedback])/self.N_k[[1,0][action]]
